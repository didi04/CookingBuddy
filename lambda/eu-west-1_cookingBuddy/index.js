...
const languageStrings = {
    'en': {
        translation: {
            ...
            FINISH_MESSAGE: 'You can ask me for another recipe or say Exit to leave',
        },
    },
    'en-US': {
        translation: {
...
const handlers = {
    ...
    'LaunchRequest': function () {
        speechOutput = this.t('WELCOME_MESSAGE', this.t('SKILL_NAME'));
    ...
    'AMAZON.StopIntent': function () {
        speechOutput = this.t('STOP_MESSAGE');
   ...
   'AMAZON.CancelIntent': function () {
    ...
    'AMAZON.NoIntent': function () {
        speechOutput = this.t('NEXT_MESSAGE');
        recipeIndex++;
        const recipe = getRecipes(ingredients, (response) => { //quer the recipes based on the ingredient(s)
            speechOutput += response.recipes[recipeIndex].title; // get the recipe's title
            if (speechOutput != null){ // check for a matching recipe
                ...
                this.response.speak(speechOutput).listen(speechOutput); // respond to the user with the recipe
                this.emit(':responseReady'); 
